...
const QuestionList = ({ data ,active}) => {
  ...
  const navigateFunction = async () => {
    await analytics().logEvent('navigate_sub_Question', {
      List: 'navigate to sub project question',
    })
    navigation.navigate(navigationString.SUBPROJECT_QUESTION);
...
//       className="border border-[#0066A2] flex-row py-2 mx-2 rounded-lg px-2 bg-white mt-1"
//     >
//       <View
//       className="border-2 border-[#0066A2] self-center rounded-full overflow-hidden h-16 w-16 "
// >
//         <Image
//           source={{
...
//         <Text className="font-semibold text-black">{data.title}</Text>
//         <Text className=" text-xs">Asked by : {data.userId.firstName}</Text>
//         <View className="flex-row">
//         <Text className=" text-xs">{new Date(data.createdAt).toLocaleDateString() + "   "}</Text>
//         <Text className=" text-xs">{new Date(data.createdAt).toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })}</Text>
//       </View>
//       </View>
//     </TouchableOpacity> : data.isSummary &&
//      <TouchableOpacity
//      onPress={navigateFunction}
//      className="border border-[#0066A2] flex-row py-2 mx-2 rounded-lg px-2 bg-white mt-1"
//    >
