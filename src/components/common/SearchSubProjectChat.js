import {
  ...
  const SearchSubProjectChat = ({ setSearch }) => {
    ...
    const { subProject } = useSelector(
        (state) => state.reducer.subProject
      );
      ...
      if (text === "") {
        ...
        let tempList = resources.filter((data) => {
          if (data.isApproved && data.userId._id !== user._id) {
            return (
              data.userId.firstName?.toLowerCase().indexOf(text.toLowerCase())  > -1 ||
            ...
            <View>
              ...
              <ScrollView contentContainerStyle={{ paddingVertical: 10 }}>
                {searchUser?.map((data, index) => {
                  return data.userId.firstName !== undefined && data.isApproved && data.userId._id !== user._id ? (
                    <TouchableOpacity
