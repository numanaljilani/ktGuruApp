...
const SubQuestionDetails = ({}) => {
  ...
  const { _id: questionId, isSummary } = useSelector(
    ...
    <View className="s bg-white flex-1 m-2 rounded-lg">
      ...
      <View className="mx-4 mt-4 pb-2 border-b-2">
        ...
        <Text className=" text-xs mt-2">
          Asked by {items.question.question?.userId?.firstName}{" "}
          {new Date(items.question.question.createdAt).toLocaleDateString() +
            " " +
            new Date(items.question.question.createdAt).toLocaleString(
              "en-US",
      ...
      <ScrollView className=" flex-1">
        <View className="mx-4 mt-2">
          ...
          <Text>{items.question.question?.desc}</Text>
          {items.question.question?.file && (
            <TouchableOpacity
            ...
            >
              ...
              <Text className="font-semibold text-[#0066A2] pl-4">
                {getFileExtention(items.question.question.file)}
              </Text>
            </TouchableOpacity>
          )}
      ...
      {commentFocus && !isSummary && (
        <View className="border-t border-[#0066A2] mb-4  px-4">
